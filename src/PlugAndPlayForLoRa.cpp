...
PlugAndPlayForLoRa::PlugAndPlayForLoRa( uint8_t cssPin,   
                                        uint8_t busyPin,
                                        uint8_t resetPin ){
    sx1280CssPin = cssPin;   
...
void PlugAndPlayForLoRa::sx1280Select(){
...
void PlugAndPlayForLoRa::sx1280Deselect(){
...
void PlugAndPlayForLoRa::sx1280Reset( ){
...
void PlugAndPlayForLoRa::zeroingAnArray( uint8_t arrayToZero[], 
...
void PlugAndPlayForLoRa::sx1280Setup( uint8_t standbyMode, 
                                      uint8_t packetType, 
    ...
    *( setupWriteData + 1 ) = standbyMode; /* Setting STDBY_RC Mode 0x01, STDBY_XOSC */
    ...
    sx1280Deselect();
    zeroingAnArray( setupWriteData, 10 );/* Reassigning all array values to 0 */
...
void PlugAndPlayForLoRa::sx1280Tx( uint8_t power, 
                                   uint8_t rampTime, 
                                   uint8_t outboundMessage[ ],
                                   uint8_t txIrq158, 
                                   uint8_t txIrq70, 
    ...
    *( txWriteData + 2 ) = rampTime; /* rampTime    */
    ...
    sx1280Deselect();
    zeroingAnArray( txWriteData, 258 );/* Reassigning all array values to 0 */
...
void PlugAndPlayForLoRa::sx1280Rx( uint8_t rxIrq158, 
                                   uint8_t rxIrq70, 
    ...
    *( rxWriteData + 8 ) = 0x00;      /* setting DIO 3 Mask bits 7:0 to 0 */
    ...
    sx1280Deselect();
    zeroingAnArray( rxWriteData, 258 ); /* Reassigning all array values to 0 */
